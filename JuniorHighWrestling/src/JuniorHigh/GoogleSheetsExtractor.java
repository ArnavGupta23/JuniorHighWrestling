...
public class GoogleSheetsExtractor {
    ...
    private static Credential getCredentials(final NetHttpTransport HTTP_TRANSPORT) throws IOException {
        // Load client secrets.
        InputStream in = GoogleSheetsExtractor.class.getResourceAsStream(CREDENTIALS_FILE_PATH);
        if (in == null) {
 ...
 */ 
	...
	private static String GRADE_5_TOKEN="G5";
	private static String GRADE_6_TOKEN="G6";
	private static String GRADE_7_TOKEN="G7";
	private static String GRADE_8_TOKEN="G8";
    ...
    public GoogleSheetsExtractor(String t, String sid) {
		setTeam(t);
		setSheetId(sid);
	    try {
...
System.out.println("mString->" + mString);
	...
	private int processTourneyHeaderRows(List<List<Object>> resultSheet) throws ExcelExtractorException {
	   ...
	   if ( ! isABlankRow(blankRow) ) {
		   throw new ExcelExtractorException ("expecting blank row at " + (rowNumAt+rowCheck),rowNumAt);
	   }
				...
				if ( rowAt.size() >= 5 ) {
					...
					//	verboseMessage( nameCell + wtClassVal + genderVal + gradeVal + recordVal );
						...
						if ( t.wrestlerExists(nameCell) ) {
							...
							//adding wrestler into team object.
							//doing the connection one time before the loop
							//passing value result connection which was created outside the loop
							extractResults(aWrestler, valuesResult);
